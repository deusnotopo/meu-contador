...
export const detectPatterns = (
  ...
  const reminderDescriptions = new Set(
    existingReminders.map((r) => r.description.toLowerCase().trim())
  );

  Object.entries(groups).forEach(([desc, list]) => {
    if (list.length < 2) return; // Need at least 2 occurrences to call it a pattern

    // Sort by date to check interval
    list.sort(
    ...
    if (isMonthly || list.length >= 3) {
      patterns.push({
        ...
        amount: list[list.length - 1].amount, // Use most recent amount
