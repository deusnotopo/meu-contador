...
export const VoiceCommander = () => {
  ...
  const confirmAction = () => {
    ...
    if (parsedResult.type === "transaction") {
      const t = parsedResult.data;
    ...
    } else if (parsedResult.type === "investment") {
      const inv = parsedResult.data;
      addAsset({
        name: inv.ticker, // Default name to ticker if not provided
        ticker: inv.ticker,
        type: inv.type || "stock",
        amount: inv.amount,
      ...
      showSuccess(`Investimento em ${inv.ticker} registrado!`);
    }
          ...
          >
            <Card className="glass-premium border-indigo-500/50 shadow-2xl shadow-indigo-500/20">
              <CardContent className="p-4 space-y-4">
                ...
                {parsedResult.type === "transaction" ? (
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span
                        className={`font-bold ${
                          parsedResult.data.type === "income"
                            ? "text-emerald-400"
                            : "text-red-400"
                        }`}
                      >
                        R$ {parsedResult.data.amount?.toFixed(2)}
                      </span>
                    </div>
                    <div className="flex justify-between">
                ...
                ) : parsedResult.type === "investment" ? (
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      <span className="text-slate-400">Ticker:</span>
                      <span className="font-bold text-indigo-400">
                        {parsedResult.data.ticker}
                      </span>
                    </div>
                    <div className="flex justify-between">
                    ...
                    {parsedResult.data.price && (
                      <div className="flex justify-between">
                        ...
                        <span className="text-emerald-400 font-bold">
                          R$ {parsedResult.data.price?.toFixed(2)}
                        </span>
                      </div>
                    )}
                  </div>
