...
export const DebtRecovery = () => {
  ...
  const handleAdd = async () => {
    if (!newDebt.name.trim()) {
      showError("Nome da dívida é obrigatório.");
      return;
    }
    if (newDebt.balance <= 0) {
      showError("Saldo deve ser maior que zero.");
      return;
    }
  ...
  const totalInterestPerMonth = debts.reduce(
    (s, d) => s + (d.balance * d.interestRate) / 100,
    0,
  );
  ...
  const estimateMonths = (d: Debt, extra: number): number => {
    const payment =
      (d.minPayment ?? 0) + (sortedDebts[0]?.id === d.id ? extra : 0);
    if (payment <= 0) return 999;
    const r = d.interestRate / 100;
    if (r === 0) return Math.ceil(d.balance / payment);
    return Math.ceil(
      Math.log(payment / (payment - d.balance * r)) / Math.log(1 + r),
    );
  };
  ...
  return (
    ...
    >
      ...
      <motion.div variants={fadeUp}>
        <div
          className="relative overflow-hidden rounded-3xl p-6 border border-rose-500/20"
          style={{
            background:
        ...
        >
          ...
          <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-5">
            <div>
              ...
              <p className="text-xs text-white/40 mt-1.5 max-w-sm leading-relaxed">
                Priorize o pagamento das suas dívidas para economizar anos em
                juros bancários.
              </p>
            </div>
            ...
            <div className="flex gap-3 shrink-0 flex-wrap">
              {[
                {
                  label: "Total Devido",
                  value: <PrivacyValue value={totalBalance} />,
                  color: "rose",
                },
                {
                  label: "Pgto. Mínimo",
                  value: <PrivacyValue value={totalMinPayment} />,
                  color: "amber",
                },
                {
                  label: "Juros/Mês",
                  value: formatCurrency(totalInterestPerMonth),
                  color: "orange",
                },
              ].map((k) => (
                <div
                  key={k.label}
                  className={`flex flex-col items-center p-3 rounded-2xl bg-${k.color}-500/10 border border-${k.color}-500/20 min-w-[80px]`}
                >
          ...
          <div className="relative z-10 mt-5 pt-4 border-t border-white/5 flex justify-between items-center">
            <span className="text-[9px] text-white/20 uppercase tracking-widest font-bold">
              {debts.length} dívida{debts.length !== 1 ? "s" : ""} mapeada
              {debts.length !== 1 ? "s" : ""}
            </span>
            <button
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <Field
                ...
                onChange={(v) =>
                  setNewDebt((d) => ({ ...d, balance: parseFloat(v) || 0 }))
                }
              />
              <Field
                label="Juros Mensal (%)"
              ...
              <Field
                ...
                onChange={(v) =>
                  setNewDebt((d) => ({ ...d, minPayment: parseFloat(v) || 0 }))
                }
              />
            </div>
            ...
            <div className="flex gap-3 pt-1">
              <button
                ...
                className="flex-1 h-11 rounded-2xl bg-white/5 hover:bg-white/10 text-white/40 text-[10px] font-black uppercase tracking-widest transition-all"
              >
                Cancelar
              </button>
              <button
                onClick={handleAdd}
                className="flex-1 h-11 rounded-2xl bg-rose-600 hover:bg-rose-500 text-white text-[10px] font-black uppercase tracking-widest transition-all active:scale-95"
              >
                Registrar Dívida
              </button>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
            ...
            <motion.div variants={fadeUp}>
              <div className="text-center py-16 rounded-3xl border border-white/5 bg-white/[0.01]">
                ...
                <div className="text-xs text-white/30">
                  Clique em "Registrar Dívida" para mapear suas pendências
                  financeiras.
                </div>
              </div>
            </motion.div>
              ...
              {sortedDebts.map((debt, idx) => {
                ...
                return (
                  ...
                  >
                    {/* Rank */}
                    <div
                      className="w-10 h-10 rounded-2xl flex items-center justify-center text-sm font-black shrink-0 border"
                      style={{
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2 flex-wrap">
                        <div>
                          ...
                          <div className="text-[10px] text-white/30 mt-0.5 flex items-center gap-2">
                            <span>
                              {CATEGORY_ICONS[debt.category] ?? "💸"}{" "}
                              {debt.interestRate}% juros/mês
                            </span>
                            <span>·</span>
                            <span>
                            ...
                            {months < 999 && (
                              <>
                                <span>·</span>
                                <span>~{months} meses para quitar</span>
                              </>
                            )}
                          </div>
                        </div>
                      ...
                      <div className="mt-3">
                        ...
                        <div className="text-[8px] text-white/20 mt-1">
                          Custo de juros este mês:{" "}
                          <span style={{ color: urgency.color }}>
                            {formatCurrency(
                              (debt.balance * debt.interestRate) / 100,
                            )}
                          </span>
                        </div>
                      </div>
        ...
        <div className="space-y-4">
          ...
          <motion.div variants={fadeUp}>
            <div
              className="rounded-3xl p-5 border border-indigo-500/20 space-y-5"
              style={{
                background:
                  "linear-gradient(135deg, rgba(99,102,241,0.07), rgba(99,102,241,0.02))",
              }}
            >
              <div className="flex items-center gap-2">
              ...
              {sortedDebts.length > 0 && (
                <div>
                  ...
                  <div
                    className="p-3 rounded-2xl flex items-center justify-between border border-indigo-500/20"
                  ...
                  >
                    <div>
                      ...
                      <div className="text-[9px] text-white/30">
                        {sortedDebts[0]?.interestRate}% ao mês
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      {extraPayment > 0 && (
          ...
          <motion.div variants={fadeUp}>
            ...
            >
              ...
              <p className="text-[11px] text-amber-200/50 leading-relaxed">
                O rotativo do cartão pode chegar a{" "}
                <strong className="text-amber-400">400% ao ano</strong> no
                Brasil. Se sua taxa mensal for <strong>acima de 8%</strong>,
                considere consolidar com um empréstimo pessoal de taxa menor.
              </p>
              ...
              <div className="rounded-3xl p-5 border border-white/[0.06] space-y-3 bg-white/[0.015]">
                ...
                {[
                  ...
                  },
                  {
                    ...
                    value: formatCurrency(totalInterestPerMonth * 12),
                    icon: "📅",
                  },
                ].map((m) => (
                  <div key={m.label} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-[9px] text-white/25 uppercase tracking-wider font-bold">
                        {m.label}
                      </div>
                    </div>
