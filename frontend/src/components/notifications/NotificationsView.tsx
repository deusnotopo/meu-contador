...
const ReminderRow = ({
  ...
  return (
    ...
    >
      <div
        className={`w-2 h-2 rounded-full shrink-0 ${r.isPaid ? "bg-emerald-400" : isOverdue ? "bg-rose-400 animate-pulse" : "bg-amber-400"}`}
      />
      <div className="flex-1 min-w-0">
        ...
        <div className="text-[10px] text-neutral-500 font-mono">
          ...
          {r.recurring
            ? RECURRING_LABELS[r.recurring] || r.recurring
            : "Uma vez"}
        </div>
      </div>
      <div className="flex items-center gap-2 shrink-0">
        <span
          className={`text-[12px] font-black font-mono ${r.isPaid ? "text-emerald-400" : isOverdue ? "text-rose-400" : "text-white"}`}
        >
          {fmt(r.amount)}
        </span>
...
export const NotificationsView = ({ onBack }: NotificationsViewProps) => {
  ...
  useEffect(() => {
    ...
    const handleRecur = (e: Event) => setRecurringAlerts((e as CustomEvent<string[]>).detail);
  ...
  const handleRemove = async (id: string) => {
    try {
      ...
      showError("Não foi possível excluir.");
    }
  };
    ...
    <div className="pt-2 pb-24 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="text-[10px] uppercase font-black tracking-[0.2em] text-blue-400/80 mb-1">
        ...
        <div className="grid grid-cols-3 gap-3 mb-8">
          {(["danger", "warning", "success"] as const).map((type) => {
            ...
            return (
              <div
                key={type}
                className={`card-obsidian px-4 py-3 rounded-2xl flex flex-col items-center justify-center border-white/5 ${cfg.bg}`}
              >
                <div
                  className={`text-2xl font-black font-mono tracking-tighter ${cfg.text}`}
                >
                  {count}
                </div>
                <div
                  className={`text-[8px] font-black uppercase tracking-widest opacity-80 mt-1 text-center ${cfg.text}`}
                >
                  {cfg.label}
                </div>
              </div>
          ...
          <div className="relative z-10 flex gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-white/50 leading-relaxed mb-4 max-w-sm">
                Receba insights e alertas de limite diretamente na tela
                bloqueada do seu celular.
              </p>
              <button
                onClick={subscribe}
                disabled={loading}
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-3">
            ...
            {unreadCount > 0 && (
              <button
                onClick={markAllRead}
                className="text-[10px] text-blue-400/70 hover:text-blue-400 font-black uppercase tracking-widest cursor-pointer"
              >
                Marcar todas
              </button>
            )}
          ...
          <AnimatePresence>
            {backendNotifs.slice(0, 8).map((n) => (
              ...
              >
                <div
                  className={`w-2 h-2 rounded-full mt-1.5 shrink-0 ${n.readAt ? "bg-white/20" : "bg-blue-400 animate-pulse"}`}
                />
                <div className="flex-1 min-w-0">
                  <div className="text-[13px] font-bold text-white truncate">
                    {n.title}
      ...
      <div className="mb-8">
              ...
              <div className="card-obsidian rounded-2xl p-4 border border-blue-500/15 bg-gradient-to-br from-blue-500/5 to-transparent space-y-3">
                ...
                <div className="flex gap-2">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setForm((f) => ({ ...f, recurring: e.target.value }))
                    }
                    className="flex-1 bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-[13px] text-white outline-none focus:border-blue-500/50 transition-all"
                  >
        ...
        {remLoading ? (
          ...
          <p className="text-[12px] text-neutral-600 py-3">
      ...
      {recurringAlerts.map((name) => (
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          onClick={() =>
            setRecurringAlerts((prev) => prev.filter((r) => r !== name))
          }
        ...
        >
          ...
          <div className="relative z-10 flex gap-4 items-start">
            ...
            <div>
              <div className="text-[9px] font-black uppercase tracking-[0.15em] text-purple-400 mb-1">
              ...
              <p className="text-xs text-white/50 leading-relaxed font-medium">
                Nosso detector percebeu múltiplos gastos idênticos. Deseja
                registrar como gasto recorrente?
              </p>
            </div>
          </div>
        </motion.div>
      ...
      {activeAlerts.length > 0 ? (
          ...
          <AnimatePresence>
            {activeAlerts.map((n) => {
              ...
              return (
                ...
                >
                  <div
                    className={`absolute top-0 right-0 w-32 h-32 blur-[40px] opacity-10 group-hover:opacity-30 transition-opacity duration-500 pointer-events-none rounded-full ${cfg.bg}`}
                  />

                  <div
                    className={`relative z-10 flex h-full ${isDanger ? "flex-col md:flex-row gap-6 items-start md:items-center" : "flex-col gap-4"}`}
                  >
                    <div className="flex items-start justify-between w-full">
                      <div
                        className={`flex items-center justify-center shrink-0 border shadow-inner ${isDanger ? "w-14 h-14 rounded-2xl" : "w-10 h-10 rounded-xl"} ${cfg.bg} ${cfg.border} ${cfg.text}`}
                      >
                    ...
                    <div className="flex-1 flex flex-col justify-between h-full w-full">
                      <div className="mb-2">
                        <div className="flex items-start justify-between gap-4">
                          <h3
                            className={`${isDanger ? "text-lg md:text-xl" : "text-sm"} font-black text-white leading-tight`}
                          >
                            {n.title}
                          </h3>
                          {isDanger && (
                        ...
                        <p
                          className={`text-xs text-white/50 leading-relaxed font-medium mt-1.5 ${isDanger ? "max-w-xl" : ""}`}
                        >
                          {n.message}
                        </p>
                      </div>
                      ...
                      {targetTab && (
                        <div
                          className={`flex items-center gap-1.5 text-[9px] font-black uppercase tracking-[0.1em] mt-auto pt-4 ${cfg.text}`}
                        >
                          <div
                            className={`w-1.5 h-1.5 rounded-full ${cfg.bg} ring-1 ring-[currentcolor]`}
                          />
        ...
        >
          ...
          <h2 className="text-lg font-black text-white mb-2">
            Monitoramento Ativo. Zero Alertas.
          </h2>
