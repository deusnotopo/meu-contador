...
const inputClass =
  "w-full bg-white/[0.04] border border-white/10 rounded-xl px-3.5 py-3 text-sm text-white outline-none focus:border-blue-500/50 transition-colors mb-2 appearance-none block";
...
export const ProvisaoView: React.FC<Props> = ({ onBack }) => {
  const { goals, addGoal, deleteGoal, updateGoalProgress, loading } =
    useGoals();
  ...
  const proximasStr = provisoes
    ...
    .sort(
      (a, b) => ((a.mes - mesAtual + 12) % 12) - ((b.mes - mesAtual + 12) % 12),
    );
  ...
  return (
    <div className="pt-2.5 pb-24 animate-in fade-in duration-300">
      ...
      {/* Tip */}
      <div
        className="nudge mb-4"
        style={{
          borderLeftColor: "#4A8BFF",
          borderColor: "rgba(74,139,255,0.2)",
          background: "rgba(74,139,255,0.05)",
        }}
      >
        <div className="nudge-ttl" style={{ color: "#4A8BFF" }}>
          💡 Como funciona
        </div>
        <div className="nudge-body">
          Divide despesas anuais (IPTU, IPVA, férias, seguro) em 12 parcelas
          mensais e separa o valor todo mês. Assim você sempre tem o dinheiro na
          hora certa. <strong>Zero surpresas.</strong>
        </div>
      </div>
      ...
      {provisoes.length > 0 && (
        <div className="grid grid-cols-6 gap-3 mb-5">
          ...
          <div className="col-span-6 rounded-[24px] px-6 py-5 bg-blue-500/[0.08] border border-blue-500/20 backdrop-blur-md flex justify-between items-center relative overflow-hidden">
            ...
            <div>
              <div className="text-[11px] font-bold text-blue-400 uppercase tracking-widest mb-1">
                Separar este mês
              </div>
              <div className="text-[28px] font-black text-[var(--t1)] tracking-tight">
                {fmt(totalMensal)}
              </div>
            </div>
          ...
          <div className="col-span-3 bg-[var(--glass)] border border-[var(--border)] rounded-[20px] p-4 flex flex-col justify-center">
            <div className="text-[10px] font-semibold text-[var(--t3)] uppercase mb-1">
              Acumulado
            </div>
          ...
          <div className="col-span-3 bg-[var(--glass)] border border-[var(--border)] rounded-[20px] p-4 flex flex-col justify-center">
            ...
            <div className="text-[18px] font-extrabold text-[var(--t1)]">
              {fmt(totalMeta)}
            </div>
          </div>
        </div>
              ...
              >
                ...
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-3">
                    <div className="w-11 h-11 rounded-[14px] bg-white/5 flex items-center justify-center text-2xl">
                      {p.emoji}
                    </div>
                    <div>
                      ...
                      <div
                        className={`text-[11px] font-semibold ${isUrgent ? "text-[var(--red)]" : "text-[var(--amber)]"}`}
                      >
                        {isUrgent
                          ? "Vence mês que vem!"
                          : `Vence em ${diff} meses`}
                  ...
                  <div className="text-right">
                    <div className="text-[15px] font-black font-mono text-[var(--t1)]">
                      {fmt(falta)}
                    </div>
                    <div className="text-[10px] text-[var(--t3)] font-semibold">
                      restantes
                    </div>
                  </div>
                </div>
      ...
      {showSugestoes && (
        <div className="card mb-3">
          <div className="text-[12px] text-[var(--t2)] mb-2.5 font-semibold">
            Toque para adicionar:
          </div>
          <div className="flex flex-wrap gap-2">
            {SUGESTOES_BR.filter(
              (s) => !provisoes.find((p) => p.nome === s.nome),
            ).map((s) => (
              <button
                key={s.nome}
              ...
              >
                {s.emoji} {s.nome}
                <span className="text-[var(--t3)] text-[10px] font-mono">
      ...
      {showForm && (
        <div className="card mb-3">
          ...
          <input
            ...
            onChange={(e) =>
              setForm((f) => ({ ...f, valorAnual: e.target.value }))
            }
            className={inputClass}
          />
          <select
          ...
          {form.valorAnual && parseFloat(form.valorAnual) > 0 && (
            <div className="text-[12px] text-[var(--t3)] mb-2.5">
              Você vai separar{" "}
              <strong style={{ color: "#4A8BFF" }}>
                {fmt(parseFloat(form.valorAnual) / 12)}/mês
              </strong>{" "}
              para isso.
            </div>
          )}
        ...
        {provisoes.map((p) => {
          ...
          return (
            ...
            >
              ...
              <div>
                <div className="font-extrabold text-[14px] text-[var(--t1)] tracking-tight">
                  {p.nome}
                </div>
                <div className="text-[10px] text-[var(--t3)] font-semibold mt-0.5">
                  Vence em {mesVenc} ·{" "}
                  <span className="font-mono">{fmt(mensal)}/mês</span>
                </div>
              </div>
              ...
              <div className="mt-auto">
                <div className="h-1 bg-white/5 rounded-full overflow-hidden mb-1.5">
                  <div
                    ...
                    style={{
                      width: `${Math.min(100, pct * 100)}%`,
                      background: isDone ? "var(--green)" : p.cor,
                    }}
                  />
                </div>
      ...
      {provisoes.length === 0 && !showForm && !showSugestoes && (
        <div className="card text-center p-8">
          ...
          <div className="font-bold mb-1.5 text-[var(--t1)]">
            Chega de surpresas financeiras
          </div>
          <div className="text-[13px] text-[var(--t3)] mb-4 leading-relaxed">
            Adicione suas despesas anuais (IPTU, IPVA, férias, etc.) e comece a
            separar mensalmente.
          </div>
          <div className="flex gap-2">
            <button
      ...
      {provisoes.length > 0 && (
        <div className="text-center py-4 text-[12px] text-[var(--t3)] leading-relaxed">
          Separe{" "}
          <strong className="text-[var(--t2)]">{fmt(totalMensal)}</strong> todo
          mês e nunca mais seja pego de surpresa. 🎯
        </div>
      )}
    </div>
  );
