    ...
    <div className="space-y-8 pt-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          {[
            ...
            <SelectCard 
              ...
              onClick={() => handleProfileChange("riskProfile", exp.id === "beginner" ? "conservative" : "moderate")}
              icon={exp.icon} 
              label={exp.label} 
      ...
      <div className="space-y-4">
        ...
        <div className="flex gap-2 p-1 bg-white/5 rounded-2xl mx-auto w-fit border border-white/5">
          {["conservative", "moderate", "aggressive"].map(mode => (
            <button 
              key={mode}
              onClick={() => handleProfileChange("riskProfile", mode as "conservative" | "moderate" | "aggressive")}
      ...
      <div className="space-y-4 text-center">
        ...
        <div className="bg-white/5 p-6 rounded-[2rem] border border-white/10">
          <div className="text-4xl font-black tabular-nums text-indigo-400 tracking-tighter">
            {formatCurrency(
              investments.reduce((sum, investment) => sum + ((investment.amount || 0) * (investment.averagePrice || 0)), 0)
            )}
          </div>
      ...
      <div className="space-y-3">
        {investments.map((investment, index) => (
          <div key={investment.id} className="rounded-2xl border border-white/10 bg-white/5 p-4 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <button
                type="button"
                onClick={() => setInvestments((prev) => prev.filter((item) => item.id !== investment.id))}
            ...
            <div className="grid grid-cols-2 gap-3">
              <input
                ...
                placeholder="Ticker"
                onChange={(e) => setInvestments((prev) => prev.map((item) => item.id === investment.id ? { ...item, ticker: e.target.value.toUpperCase(), name: item.name || e.target.value.toUpperCase() } : item))}
                className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm text-white"
              />
              <select
                value={investment.type}
                onChange={(e) => setInvestments((prev) => prev.map((item) => item.id === investment.id ? { ...item, type: e.target.value as OnboardingInvestment['type'] } : item))}
                className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm text-white"
              >
              ...
              <input
                ...
                placeholder="Qtd"
                onChange={(e) => setInvestments((prev) => prev.map((item) => item.id === investment.id ? { ...item, amount: Number(e.target.value) } : item))}
                className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm text-white"
              />
              <input
                type="number"
                ...
                onChange={(e) => setInvestments((prev) => prev.map((item) => item.id === investment.id ? { ...item, averagePrice: Number(e.target.value), currentPrice: Number(e.target.value) } : item))}
                className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm text-white"
              />
            </div>
          </div>
        ...
        <div className="flex gap-2">
          <button
            type="button"
            onClick={() => setInvestments((prev) => [...prev, {
              id: Date.now(),
              name: '',
              ticker: '',
              type: 'stock',
