...
import { Car, Loader2, Plus, AlertCircle, RefreshCw, Trash2, Search } from 'lucide-react';
...
const SearchableSelect = ({ options, onSelect, placeholder, loading }: {
  options: Option[];
  ...
  const filtered = useMemo(() => {
    ...
    return options.filter(o => o.label.toLowerCase().includes(query.toLowerCase())).slice(0, 50);
  }, [options, query]);

  if (loading) return (
    <div className="flex items-center gap-2 py-4 text-blue-400/80 text-sm">
      <Loader2 className="animate-spin" size={16} />
      <span className="font-mono tracking-wider text-[11px] uppercase">Consultando FIPE...</span>
    </div>
  );
        ...
        <div className="absolute z-50 top-full mt-1 w-full max-h-52 overflow-y-auto bg-[#0d1524] border border-white/10 rounded-xl shadow-2xl">
          {filtered.map(o => (
            <button
              ...
              onMouseDown={() => { onSelect(o); setQuery(o.label); setOpen(false); }}
            >
              {o.label}
            </button>
          ))}
...
const StepProgress = ({ current, total }: { current: number; total: number }) => (
  <div className="flex items-center gap-1.5 mb-5">
    ...
    <span className="text-[10px] text-white/40 font-mono ml-1">{current}/{total}</span>
  </div>
);
...
export const FipeVehicleManager = ({ onSaved }: { onSaved?: () => void }) => {
  ...
  const handleTypeSelect = (t: VehicleType) => {
    setType(t);
    setBrand(null); setModel(null); setPriceData(null);
    setModels([]); setYears([]);
    setStep(2);
  };
  ...
  const handleBrandSelect = async (b: Option) => {
    setBrand(b);
    setModel(null); setPriceData(null);
    setModels([]); setYears([]);
    setStep(3);
  ...
  const handleYearSelect = async (y: Option) => {
    ...
    const res = await fipe.getSpecificPrice(type, brand!.value, model!.value, y.value);
    setPriceData(res);
  ...
  return (
    <div className="rounded-3xl overflow-hidden" style={{
      background: 'linear-gradient(180deg, rgba(8,14,31,1) 0%, rgba(3,7,18,1) 100%)',
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-[15px] font-bold text-white flex items-center gap-2">
              Garagem Mágica
              <span className="bg-blue-500 px-1.5 py-0.5 rounded text-[8px] font-black uppercase text-white shadow-[0_0_10px_rgba(59,130,246,0.4)]">FIPE</span>
            </div>
            <div className="text-[11px] text-neutral-500">Bens físicos avaliados pela tabela oficial FIPE</div>
          </div>
        </div>
        <button
          onClick={() => { setShowWizard(v => !v); if (!showWizard) resetWizard(); }}
      ...
      {vehicleAssets.length > 0 && (
        <div className="relative z-10 p-5">
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-bold text-white truncate" title={v.name}>{v.name}</div>
                    <div className="flex items-center gap-2 mt-0.5">
                      <span className="text-[10px] text-blue-400/70 font-mono bg-blue-500/10 px-2 py-0.5 rounded-full">{v.ticker}</span>
                      <span className="text-[10px] text-white/40">{(v as any).platform}</span>
                    </div>
                  </div>
                  <div className="text-right shrink-0">
                    <div className="text-sm font-black font-mono text-white">{formatCurrency(priceNum)}</div>
                    <div className="text-[9px] text-white/30 mt-0.5">Valor de Mercado</div>
                  </div>
                  <button
          ...
          <div className="mt-4 pt-3 border-t border-white/5 flex justify-between items-center">
            <span className="text-[10px] uppercase text-white/30 tracking-widest font-bold">Total de Bens Físicos</span>
            <span className="text-sm font-black font-mono text-blue-300">
              {formatCurrency(vehicleAssets.reduce((s, v) => s + (v.currentPrice || v.averagePrice), 0))}
            </span>
          </div>
        </div>
      )}
      ...
      {vehicleAssets.length === 0 && !showWizard && (
        <div className="relative z-10 p-10 text-center">
          ...
          <div className="text-sm font-bold text-white/60 mb-1">Nenhum bem físico cadastrado</div>
          <div className="text-[11px] text-white/30">Clique em "Adicionar" para registrar seu carro ou moto pela FIPE.</div>
        </div>
      )}
              ...
              <AnimatePresence mode="wait">
                ...
                {step === 1 && (
                  <motion.div key="s1" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }}>
                    <div className="text-xs text-white/40 uppercase tracking-widest mb-1">Etapa 1</div>
                    <div className="text-[15px] font-bold text-white mb-4">Que tipo de veículo?</div>
                    <div className="grid grid-cols-3 gap-3">
                      {([['carros', '🚗', 'Carro'], ['motos', '🏍️', 'Moto'], ['caminhoes', '🚛', 'Caminhão']] as [VehicleType, string, string][]).map(([t, icon, label]) => (
                        <button
                          key={t}
                ...
                {step === 2 && (
                  <motion.div key="s2" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }}>
                    <div className="flex items-center justify-between mb-1">
                      <div className="text-xs text-white/40 uppercase tracking-widest">Etapa 2 — Fabricante</div>
                      <button onClick={() => setStep(1)} className="text-[10px] text-neutral-500 hover:text-white flex items-center gap-1 transition-colors">
                        <RefreshCw size={10} /> Voltar
                      </button>
                    </div>
                    <div className="text-[15px] font-bold text-white mb-4">Qual a marca?</div>
                    <SearchableSelect
                      options={brands}
                ...
                {step === 3 && (
                  <motion.div key="s3" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }}>
                    <div className="flex items-center justify-between mb-1">
                      <div className="text-xs text-white/40 uppercase tracking-widest">Etapa 3 — Modelo</div>
                      <button onClick={() => setStep(2)} className="text-[10px] text-neutral-500 hover:text-white flex items-center gap-1 transition-colors">
                        <RefreshCw size={10} /> Voltar
                      </button>
                    </div>
                    <div className="text-[15px] font-bold text-white mb-1">Qual o modelo?</div>
                    <div className="text-[11px] text-blue-400/70 mb-3 font-mono">Marca: {brand?.label}</div>
                    <SearchableSelect
                      options={models}
                ...
                {step === 4 && (
                  <motion.div key="s4" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }}>
                    <div className="flex items-center justify-between mb-1">
                      <div className="text-xs text-white/40 uppercase tracking-widest">Etapa 4 — Ano</div>
                      <button onClick={() => setStep(3)} className="text-[10px] text-neutral-500 hover:text-white flex items-center gap-1 transition-colors">
                        <RefreshCw size={10} /> Voltar
                      </button>
                    </div>
                    <div className="text-[15px] font-bold text-white mb-1">Ano do veículo?</div>
                    <div className="text-[11px] text-blue-400/70 mb-3 font-mono">{brand?.label} — {model?.label}</div>
                    {loadingYears ? (
                      <div className="flex items-center gap-2 py-4 text-blue-400/80 text-sm">
                        <Loader2 className="animate-spin" size={16} />
                        <span className="font-mono tracking-wider text-[11px] uppercase">Consultando FIPE...</span>
                ...
                {/* STEP 5: Result */}
                {step === 5 && (
                  <motion.div key="s5" initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}>
                    {loadingPrice ? (
                      <div className="py-12 flex flex-col items-center gap-3 text-blue-400/80">
                        <Loader2 className="animate-spin" size={32} />
                        <span className="font-mono text-[11px] uppercase tracking-widest">Avaliando pela FIPE...</span>
                      </div>
                    ) : savedOk ? (
                      <div className="py-10 text-center">
                        <div className="text-5xl mb-3">✅</div>
                        <div className="text-sm font-bold text-emerald-400">Veículo adicionado ao patrimônio!</div>
                      </div>
                    ) : priceData ? (
                      <div>
                        <div className="flex items-center justify-between mb-4">
                          <div className="text-xs text-white/40 uppercase tracking-widest">Avaliação Oficial FIPE</div>
                          <button onClick={() => setStep(4)} className="text-[10px] text-neutral-500 hover:text-white flex items-center gap-1 transition-colors">
                            <RefreshCw size={10} /> Voltar
                          </button>
                        </div>
                        ...
                        <div className="bg-blue-500/10 border border-blue-500/20 rounded-2xl p-5 mb-4 text-center relative overflow-hidden">
                          ...
                          <div className="text-[10px] uppercase tracking-widest text-blue-400/70 font-bold mb-1">Valor de Mercado</div>
                          <div className="text-3xl font-black font-mono text-white tracking-tight drop-shadow-[0_0_20px_rgba(59,130,246,0.5)]">
                            {priceData.Valor}
                          </div>
                          <div className="text-[10px] text-white/30 mt-1">Referência: {priceData.MesReferencia}</div>
                        </div>
                        ...
                        <div className="space-y-2 mb-5 text-[12px]">
                          {[
                            ...
                            <div key={k} className="flex justify-between items-baseline">
                              ...
                              <span className="font-medium text-white text-right max-w-[60%] truncate" title={v}>{v}</span>
                            </div>
                          ))}
                        </div>
                        ...
                        <div className="flex gap-3">
                          <button
                            onClick={() => { resetWizard(); setShowWizard(false); }}
                          ...
                          >
                            {isSaving ? <Loader2 size={16} className="animate-spin" /> : <><Plus size={16} /> Adicionar ao Patrimônio</>}
                          </button>
                        </div>
                      </div>
                    ) : null}
