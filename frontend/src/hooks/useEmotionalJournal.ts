...
export function useEmotionalJournal() {
  ...
  const insights = useMemo((): EmotionalInsight[] => {
    ...
    const happyEntries = entries.filter((e) => e.emotion === 'happy' || e.emotion === 'proud');
    const stressEntries = entries.filter((e) => e.emotion === 'stressed' || e.emotion === 'anxious');

    const happySpendPercent = entries.length > 0
      ? (happyEntries.length / entries.length) * 100
      : 0;
    const stressSpendPercent = entries.length > 0
      ? (stressEntries.length / entries.length) * 100
      : 0;
    ...
    if (impulseEntries.length >= 3) {
      ...
      result.push({
        ...
        recommendation: 'Use a regra 24-48: espere um dia antes de confirmar a compra.',
        stats,
      });
    }
    ...
    if (socialPattern && socialPattern.topTriggers.includes('Redes sociais')) {
      result.push({
        ...
        description: 'Você tende a gastar mais após ver posts nas redes sociais.',
        recommendation: 'Limite o tempo em redes sociais ou siga perfis minimalistas.',
        stats,
      });
    }
  ...
  const stats = useMemo(() => {
    ...
    const regretRate = totalEntries > 0
      ? (entries.filter((e) => e.regretLevel && e.regretLevel >= 4).length / totalEntries) * 100
      : 0;

    const averageSatisfaction = entries.length > 0
      ? entries.reduce((sum, e) => sum + (e.satisfactionLevel || 3), 0) / entries.length
      : 0;
